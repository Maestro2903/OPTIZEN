...
// GET /api/blood-advice - List blood advice records
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (error) {
      logger.error('Error fetching blood advice records', error, {
        request_id: requestId,
        endpoint: '/api/blood-advice',
    ...
    logger.requestComplete('GET', '/api/blood-advice', 200, duration, requestId, {
      user_id: context.user_id,
    ...
    const duration = Date.now() - startTime
    logger.error('API error in GET /blood-advice', error, {
      request_id: requestId,
      endpoint: '/api/blood-advice',
...
// POST /api/blood-advice - Create new blood advice record
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const recordData: any = {
      ...
      blood_investigation_data: body.blood_investigation_data || {},
      advice_remarks: body.advice_remarks || null,
    ...
    if (error) {
      logger.error('Error creating blood advice record', error, {
        request_id: requestId,
        endpoint: '/api/blood-advice',
        user_id: context.user_id,
        record_number: body.record_number
    ...
    const duration = Date.now() - startTime
    logger.requestComplete('POST', '/api/blood-advice', 201, duration, requestId, {
      user_id: context.user_id,
    ...
    const duration = Date.now() - startTime
    logger.error('API error in POST /blood-advice', error, {
      request_id: requestId,
      endpoint: '/api/blood-advice',
      duration_ms: duration
    })
