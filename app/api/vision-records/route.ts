...
// GET /api/vision-records - List vision records
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (error) {
      logger.error('Error fetching vision records', error, {
        request_id: requestId,
        endpoint: '/api/vision-records',
    ...
    logger.requestComplete('GET', '/api/vision-records', 200, duration, requestId, {
      user_id: context.user_id,
    ...
    const duration = Date.now() - startTime
    logger.error('API error in GET /vision-records', error, {
      request_id: requestId,
      endpoint: '/api/vision-records',
...
// POST /api/vision-records - Create new vision record
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const recordData: any = {
      ...
      examination_data: body.examination_data || {},
    ...
    if (error) {
      logger.error('Error creating vision record', error, {
        request_id: requestId,
        endpoint: '/api/vision-records',
        user_id: context.user_id,
        record_number: body.record_number
    ...
    const duration = Date.now() - startTime
    logger.requestComplete('POST', '/api/vision-records', 201, duration, requestId, {
      user_id: context.user_id,
    ...
    const duration = Date.now() - startTime
    logger.error('API error in POST /vision-records', error, {
      request_id: requestId,
      endpoint: '/api/vision-records',
      duration_ms: duration
    })
